        ...
        with st.spinner("Searching database and executing dual-AI routing..."):
            ...
            if not lookup:
                ...
                safetyScore = float(
                    dataA.get(
                        "safety_score",
                        100 if dataA.get("safety_approved") else 0,
                    )
                )
                consensusScore = (
                    (genericMatchPTS * 0.4)
...
Safety Status: {'APPROVED' if dataA.get('safety_approved') else 'REQUIRES REVIEW'}
Safety Score: {dataA.get('safety_score', 0)}/100

DOSAGE INSTRUCTIONS:
{dataA.get('dosage_instructions', 'N/A')}

CLINICAL WARNINGS:
{dataA.get('key_warnings', 'None')}
----------------------------------------
